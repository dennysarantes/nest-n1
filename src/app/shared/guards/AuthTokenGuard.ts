...
export class AuthTokenGuard implements CanActivate {
    ...
    async canActivate(
        ...
        const accesToken = this.extrairTokenHeader(context.switchToHttp().getRequest());
        ...
        try {
            const payload = await this.jwtService.verifyAsync(accesToken, this.appConfigs.jwt_config)
            this.atribuirPayloadAoRequest(context.switchToHttp().getRequest(), payload)
        } catch (error) {
    ...
    lancarErro = (temToken:boolean) => {
        throw new UnauthorizedException(temToken ? 'Token inválido!' : 'Token inexistente');
    }

    atribuirPayloadAoRequest = (request: Request, payload) => request[TOKEN_PAYLOAD] = payload;
