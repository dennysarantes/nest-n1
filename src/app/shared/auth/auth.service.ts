...
export class AuthService {
    ...
    async login(dados:AutenticarPessoaDto){
        ...
        if (pessoaExiste?.length === 1) {
            senhaCorreta = await this.hashingService.compare(dados.password, pessoaExiste[0].passwordhash);
        }
        ...
        const accessToken = await this.gerarToken(pessoaExiste[0], this.appConfigs.jwt_config.ttl);
   ...
   async refreshToken(refreshToken) {
        if (!refreshToken) {
          ...
          try {
            this.jwtService.verify(refreshToken, {
                ...
                const accessToken = await  this.gerarToken(pessoa[0], this.appConfigs.jwt_config.ttlRefresh)
    ...
    private async buscarPessoa(email?: string, id?:string) {
        ...
        return  await this.pessoaRepository
            ...
            .select(['p.id as id', 'p.email as email', 'p.passwordHash as passwordhash', 'json_agg(r.nome) AS roles'])
